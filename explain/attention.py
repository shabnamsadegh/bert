...
def average_last_layer_by_head(attentions):
    ...
    tf.logging.info("Attention look! " + str(last_multihead_attn.shape))
    last_multihead_attn = tf.reshape(last_multihead_attn, [1,12,128,128])#todo important! these are hard coded. fix them
    tf.logging.info("Attention look! " + str(last_multihead_attn.shape))
