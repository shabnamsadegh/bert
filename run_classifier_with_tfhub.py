...
def create_model(is_training, input_ids, input_mask, segment_ids, labels,
  ...
  with tf.variable_scope("loss"):
    if is_training:
      # I.e., 0.1 dropout
      output_layer = tf.nn.dropout(output_layer, keep_prob=0.7)#0.9 I changed it to 0.7 because the topics are very close to each other and harder to learn
    ...
    tf.logging.info("Shabnam Look!" +  str(list(attns[0].values())[0].shape))
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits):
        ...
        tf.logging.info("Shabnam look!" + str(logits[0]))
